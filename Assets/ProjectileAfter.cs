...
public class ProjectleFinal : MonoBehaviour
{
    ...
    float GetCParameter(float M)
    {
        if (M > 3.5f)
            ...
            return -0.239257f * Mathf.Pow(M, 4) + 1.77419f * Mathf.Pow(M, 3) - 4.80598f * Mathf.Pow(M, 2) + 5.54821f * M - 1.92192f;
    }
    /* float K(float M)
     {
         if (M < 0.7f)
    ...
    void Update()
    {
        ...
        airForce = airResitance * velocity.sqrMagnitude * -velocity.normalized * GetCParameter(velocity.magnitude / soundSpeed) * (1 + 13 * Mathf.Deg2Rad * Vector3.Angle(velocity, transform.forward));
        ...
        velocity += Time.deltaTime * (airForce + derivation + Physics.gravity);
